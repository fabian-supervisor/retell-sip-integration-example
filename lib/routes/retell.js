...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', async(session) => {
    ...
    try {
      ...
      // Log the complete session.sip object to see all incoming SIP data
      if (session.sip) {
        logger.info({sip: session.sip}, `Complete SIP object for call ${session.call_sid}`);
      }
      
      // Extract X-DID header from incoming SIP headers if present
      if (session.sip && session.sip.headers && session.sip.headers['X-DID']) {
        headers['X-DID'] = session.sip.headers['X-DID'];
        logger.info({xDid: headers['X-DID']}, `Found X-DID header: ${headers['X-DID']}`);
      }
